(function () {
  ...
  function renderContacts(items, meta = {}) {
    if (!items.length) {
      directoryResults.innerHTML = '<div class="notice">No social support contact matched this country/search. Add a verified record in backend/data/globalDirectory.json.</div>';
      setStatus("No matching social support contact found.", "info");
      return;
    }
    ...
    directoryResults.innerHTML = items.map((item) => `
      <article class="result-card">
        <div class="result-top">
          <div>
            <h3>${escapeHtml(item.country || item.countryCode)} — ${escapeHtml(item.localName || "Social support / protection agency")}</h3>
            <p>${escapeHtml(item.categoryLabel || "Social support / child protection")}</p>
          </div>
          <span class="badge ${badgeClass(item.verificationStatus)}">${badgeLabel(item.verificationStatus)}</span>
        </div>
        <div class="result-meta"><strong>Emergency / hotline:</strong> ${escapeHtml(item.emergencyNumber || "Not listed")}</div>
        <div class="result-meta"><strong>Phone:</strong> ${escapeHtml(item.phone || "Not listed publicly")}</div>
        <div class="result-meta"><strong>Contact type:</strong> ${escapeHtml(item.contactType || "social_support_agency")}</div>
    ...
    setStatus(`${items.length} child-protection/social-support contact record${items.length === 1 ? "" : "s"} shown. ${meta.warning || "Verify locally before field use."}`, "success");
  }
  ...
  async function searchContacts() {
    ...
    const category = categorySelect.value || "social_support";
    const query = directoryInput.value.trim();
    setStatus("Loading worldwide social support contacts...", "info");
    try {
      const path = `/global-directory/contacts?country=${encodeURIComponent(country)}&category=${encodeURIComponent(category)}${query ? `&q=${encodeURIComponent(query)}` : ""}`;
      const payload = await apiGet(path);
      renderContacts(payload.results || [], payload.meta || {});
    } catch (error) {
      directoryResults.innerHTML = "";
      setStatus(error.message || "Unable to load social support contacts.", "error");
    }
  }
  ...
  categorySelect.addEventListener("change", searchContacts);
