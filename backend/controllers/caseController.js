...
import { getLanguageName, translateText } from '../services/translatorService.js';
...
function normalizeCountryCode(value = 'NG') {
  const raw = String(value || '').trim();
  if (!raw) return 'NG';
  const aliases = { nigeria: 'NG', 'united states': 'US', usa: 'US', america: 'US', 'united kingdom': 'GB', uk: 'GB', canada: 'CA' };
  const lower = raw.toLowerCase();
  if (aliases[lower]) return aliases[lower];
  const upper = raw.toUpperCase();
  return /^[A-Z]{2}$/.test(upper) ? upper : upper.slice(0, 2);
}

function normalizeCountryName(country = '', countryCode = 'NG') {
  const direct = normalizeString(country, '');
  if (direct && !/^[A-Za-z]{2}$/.test(direct)) return direct;
  try {
    return new Intl.DisplayNames(['en'], { type: 'region' }).of(countryCode) || 'Nigeria';
  } catch (error) {
    return direct || 'Nigeria';
  }
}

function normalizeLanguageCode(value = 'en', fallback = 'en') {
  const normalized = String(value || '').trim();
  if (!normalized) return fallback;
  const aliases = { english: 'en', yoruba: 'yo', hausa: 'ha', igbo: 'ig', french: 'fr', spanish: 'es', arabic: 'ar', swahili: 'sw', hindi: 'hi' };
...
export async function getCommunityRiskSuggestions(req, res) {
  try {
    ...
    return res.json({
      ...
      supportedLanguages: ['Auto detect', 'English', 'French', 'Spanish', 'Arabic', 'Portuguese', 'Chinese', 'Hindi', 'Swahili', 'More Azure-supported languages'],
    });
  } catch (error) {
...
function normalizeContact(payload = {}) {
  ...
  const phoneCountryDialCode = String(payload.phoneCountryDialCode || payload.contact?.phoneCountryDialCode || '').trim();
  const phoneCountryIso2 = String(payload.phoneCountryIso2 || payload.contact?.phoneCountryIso2 || '').trim().toUpperCase();
  const phoneCountryName = String(payload.phoneCountryName || payload.contact?.phoneCountryName || '').trim();
  const phoneNationalNumber = String(payload.phoneNationalNumber || payload.contact?.phoneNationalNumber || '').trim();
  const rawPhone = String(payload.phone || '').trim();
  const phone = rawPhone || (phoneNationalNumber ? `${phoneCountryDialCode} ${phoneNationalNumber}`.trim() : '');

  if (preference === 'email') {
    return { preference: 'email', value: email || directValue, phoneCountryDialCode, phoneCountryIso2, phoneCountryName, phoneNationalNumber };
  }
  ...
  return { preference: 'anonymous', value: '', phoneCountryDialCode, phoneCountryIso2, phoneCountryName, phoneNationalNumber };
}
...
function serializeCase(caseItem, options = {}) {
  ...
  const serialized = {
    ...
    reportLanguageCode: item.reportLanguageCode || '',
    country: item.country || 'Nigeria',
    countryCode: item.countryCode || 'NG',
    translatedIncidentDetails: item.translatedIncidentDetails || '',
    translationTargetLanguage: item.translationTargetLanguage || '',
    translationTargetLanguageName: item.translationTargetLanguage ? getLanguageName(item.translationTargetLanguage) : '',
...
export async function createCase(req, res) {
  ...
  try {
    ...
    const normalizedReportLanguageCode = normalizeLanguageCode(reportLanguageCode || reportLanguage, 'en');
    const normalizedReportLanguage = normalizeReportLanguage(reportLanguage || getLanguageName(normalizedReportLanguageCode), 'Text report');
    const normalizedCountryCode = normalizeCountryCode(countryCode || country || 'NG');
    const normalizedCountry = normalizeCountryName(country, normalizedCountryCode);
    const normalizedTranslationTargetLanguage = normalizeLanguageCode(translationTargetLanguage || 'en', 'en');
    let translatedIncidentDetails = '';
    let translationProvider = '';
    let translationStatus = 'not_requested';
    if (normalizedOriginalDescription && normalizedOriginalDescription !== 'No details shared.') {
      try {
        const translation = await translateText({
          text: normalizedOriginalDescription,
          from: normalizedReportLanguageCode,
          to: normalizedTranslationTargetLanguage,
        });
        translatedIncidentDetails = translation.translatedText || normalizedOriginalDescription;
        translationProvider = translation.provider || 'azure';
        translationStatus = translation.status || 'translated';
      } catch (translationError) {
        translatedIncidentDetails = normalizedOriginalDescription;
        translationProvider = 'azure';
        translationStatus = `failed: ${translationError.message}`.slice(0, 180);
      }
    }
    ...
    const casePayload = {
      ...
      reportLanguageCode: normalizedReportLanguageCode,
      country: normalizedCountry,
      countryCode: normalizedCountryCode,
      translatedIncidentDetails,
      translationTargetLanguage: normalizedTranslationTargetLanguage,
      translationProvider,
      translationStatus,
...
export async function getCases(_req, res) {
  try {
    const caseItems = await CaseReport.find().sort({ createdAt: -1 }).lean();
    const includeInternal = String(_req.headers['x-SafeVoice-staff'] || '').trim() === '1';
...
export async function getCaseByCaseId(req, res) {
  try {
    ...
    const includeInternal = String(req.headers['x-SafeVoice-staff'] || '').trim() === '1';
